...
fitM <- function(obj, metr="AIC", metrCut=NULL, ...) {
    ...
    if (is.null(obj@cv)) {
	stop("No features available! Did you forget to run cv()?")
    }
...
cvB <- function(obj, frm=NULL, ...) {
    ...
    mss <- foreach(i=1:length(obj@dataSign[,1])) %dopar% {
	frm <- as.formula(paste(frm, colnames(dat2)[i]))
	fit <- glm(frm, data=dat2, family=binomial(link=logit))
	cv <- cv.binary(fit)
	ms <- cv$acc.cv
	data.frame(ms, colnames(dat2)[i])
    ...
    colnames(mss)[2] <- "ID"
    #TODO: check empty
    mss <- mss[rev(order(mss[,1])),,drop=F]
    obj@cv <- mss
...
plotModel <- function(obj, ...) {
    stop("Currently defunc")
    df <- data.frame(obj@pred, GRP=obj@meta[,obj@var])
    df$x <- rank(df$fit, ties.method="first")
    print(df)
    if (obj@type == "lr") {
	g <- ggplot2::ggplot(df, aes(y=fit, x=x, col=factor(GRP))) + geom_point()  + geom_errorbar(aes(ymin=lwr, ymax=upr))
    } else {
	g <- ggplot2::ggplot(df, aes(y=fit, x=x, col=factor(GRP))) + geom_point()  + geom_errorbar(aes(ymin=lwr, ymax=upr))
